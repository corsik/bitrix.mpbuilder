...
$module = $moduleId ? new Module($moduleId) : null;
$bitrixComponentRootPath = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/components';

if ($_REQUEST['action'] == 'version_restore' && $moduleId && check_bitrix_sessid()) {
    rename($module->getRootDirPath() . '/install/_version.php', $module->getRootFileVersionPath());
}

if ($moduleId) {
    if (file_exists($module->getRootFileVersionPath())) {
        include($module->getRootFileVersionPath());
    }

    $NAMESPACE = \COption::GetOptionString($moduleId, 'NAMESPACE', '');
}

if ($_REQUEST['action'] == 'delete' && $moduleId && check_bitrix_sessid()) {
    Filesystem::rmDir($module->getRootTmpDirPath());
} elseif ($_POST['save'] && $moduleId && check_bitrix_sessid()) {
    $strError = '';
    ...
    if (!$strError && $_REQUEST['store_version']) {
        ...
        rename($module->getRootFileVersionPath(), $module->getRootDirPath() . '/install/_version.php');

        if (!file_put_contents($module->getRootFileVersionPath(), $module->getContextVersion($version))) {
            $strError .= GetMessage("BITRIX_MPBUILDER_NE_UDALOSQ_ZAPISATQ") . $module->getRootFileVersionPath() . '<br>';
        }
    }
    ...
    if (!$strError) {
        ...
        mkdir($module->getRootDirVersionPath($version), BX_DIR_PERMISSIONS, true);
        ...
        if (!$strError && $_REQUEST['components']) {
            $ar = [];
            $componentDir = opendir($module->getRootDirComponentPath()); // let's get components list
            if ($bCustomNameSpace) {
                ...
                while (false !== $item = readdir($componentDir)) {
                    if ($item == '.' || $item == '..' || !is_dir($path0 = $module->getRootDirComponentPath() . '/' . $item)) {
                        continue;
                    }
            ...
            foreach ($ar as $file) {
                ...
                $to = $module->getRootDirComponentPath() . ($bCustomNameSpace ? preg_replace('#^/[^/]+#', '', $file) : $file);
        ...
        foreach ($originalModuleFiles as $file) {
            ...
            $toFile = $module->getRootDirVersionPath($version) . $file;
            ...
            if ($file === '/install/version.php') {
                ...
                if ($_REQUEST['store_version'] && !file_put_contents($fromFile, $module->getContextVersion($version))) {
                ...
                if (!file_put_contents($toFile, $module->getContextVersion($version))) {
        ...
        if (!$strError && ($str = trim($_REQUEST['updater']))) {
            ...
            */
            $updaterFilePath = $module->getRootDirVersionPath($version) . '/updater.php';
    ...
    if (!$strError) {
        $linkFolder = $module->getTmpDirPath() . '/';
    ...
    if ($moduleId) {
        ...
        <tr>
            ...
            <td>
                ...
                <?
                if (file_exists($f = $module->getRootDirPath() . '/install/_version.php')) {
                    include($f);
        ...
        <tr>
            ...
            <td>
                ...
                if (!file_exists($module->getRootDirComponentPath())) {
                    ...
                    while (false !== $item = readdir($componentDir)) {
                        $path0 = $module->getRootDirComponentPath();
