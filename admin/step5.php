...
?>
	...
	<tr>
		...
		<td>
			<input name="version" value="<?=($v = $_REQUEST['version'] ? htmlspecialcharsbx($_REQUEST['version']) : VersionUp($arModuleVersion['VERSION']))?>"> <label><input type=checkbox name=store_version <?=$_REQUEST['store_version']?'checked':''?>> <?=GetMessage("BITRIX_MPBUILDER_OBNOVITQ_DATU_I_VERS")?></label>
			<?
	...
	<tr>
		...
		?>
			<tr>
				...
				<td>/bitrix/components/<input name=NAMESPACE id=NAMESPACE value="<?=htmlspecialcharsbx($NAMESPACE)?>" <?=$_REQUEST['components'] ? '' : 'disabled'?>></td>
			</tr>
			<tr>
				<td></td>
