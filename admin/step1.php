...
if ($_POST['save'] && check_bitrix_sessid())
{
	...
	if (!$strError)
	{
		...
		$str = str_replace(['{MODULE_CLASS_NAME}', '{MODULE_ID}', '{INCLUDE_CLASS_NAME}'], [
			$MODULE_CLASS_NAME,
			$module,
			$INCLUDE_CLASS_NAME
		], $str);
...
<form action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANG ?>" method="POST" enctype="multipart/form-data">
	...
	<tr>
		...
		<td><input name="partner_name"
					value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_name', '')) ?>"></td>
	</tr>
	<tr>
		<td><?= GetMessage("BITRIX_MPBUILDER_ADRES_VASEGO_SAYTA") ?></td>
		<td><input name="partner_uri"
					value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_uri', 'http://www.mysite.ru')) ?>">
		</td>
	</tr>
	<tr>
		<td><?= GetMessage("BITRIX_MPBUILDER_KOD_PARTNERA") ?></td>
		<td><input name="partner_code"
					value="<?= htmlspecialcharsbx(COption::GetOptionString($MODULE_ID, 'partner_code', '')) ?>"></td>
	</tr>
	<tr>
		<td></td>
	...
	<tr>
		<td></td>
		<td class=smalltext><?= GetMessage("BITRIX_MPBUILDER_KOD_MODULA_VVODITSA") ?> marketplace
			(<?= GetMessage("BITRIX_MPBUILDER_NAPRIMER") ?>
			<i>testmodule</i>).<br><?= GetMessage("BITRIX_MPBUILDER_PAPKA_MODULA_SOSTOIT") ?><i>mycompany</i>.<i>testmodule</i>.
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td valign=top><?= GetMessage("BITRIX_MPBUILDER_VYBERITE_KOMPONENTY") ?>
			<br>(<?= GetMessage("BITRIX_MPBUILDER_IZ_PAPKI") ?>
			/bitrix/components <?= GetMessage("BITRIX_MPBUILDER_KROME_SISTEMNYH_V") ?> bitrix)
		</td>
		<td>
			<?
			$strSelect = '';
